<template>
  <div class="add-card">
    <header class="add-header">
      <el-page-header content="编辑月卡" @back="$router.back()" />
    </header>
    <main class="add-main">
...
<script>
// 导入 编辑月卡信息接口 获取月卡详情 接口
import { getMonthItemAPI, editMonthItemAPI } from '@/api/month'
...
export default {
  ...
  methods: {
    ...
    async getMonthItem() {
      ...
      this.formData2.payTime = [res.data.cardStartDate, res.data.cardEndDate]
      ...
      this.cardInfor.cardInfor = res.data.cardInfor
      // console.log(res)
    }
  }
}
